...
async function cargarItems() {
  ...
  try {
    ...
    if (verificacion && verificacion.existe === true) {
      ...
      // Use throw to stop execution and show error message
      throw new Error(verificacion.mensaje);
    }

    // 2. Si no hay registros, cargar items
    const response = await fetch(`${scriptURL}?api=articulos&tipo=${encodeURIComponent(datosUsuario.tipo)}&categoria=${encodeURIComponent(datosUsuario.categoria)}&sede=${encodeURIComponent(datosUsuario.sede)}`);
    ...
    const filtrados = items.filter(item => {
      ...
      // Enhanced sede matching to handle case variations and ensure ALL items are included
      const sedeMatch = itemSede === searchSede || itemSede === 'ALL' || itemSede === 'all' || itemSede === 'All';
