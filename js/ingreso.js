...
// Replace the fetchModoFromAPI function with this optimized version
function getModoFromCache(tipo, categoria) {
    console.log(`Getting mode from cache for tipo: "${tipo}" and categoria: "${categoria}"`);
    
    if (!inventoryDataCache || !inventoryDataCache.length) {
        console.warn('No inventory data in cache');
        return getDefaultModo(tipo, categoria);
    }
    
    // Find matching items in the cache
    const matchingItems = inventoryDataCache.filter(item => 
        item.TIPO === tipo && 
        item.CATEGORIA === categoria
    );
    
    console.log(`Found ${matchingItems.length} matching items in cache`);
    
    // If we have matching items with a MODO field, use the first one
    if (matchingItems.length > 0 && matchingItems[0].MODO) {
        return matchingItems[0].MODO;
    }
...
// Default mode logic
function getDefaultModo(tipo, categoria) {
    console.log(`Using default mode for tipo: "${tipo}", categoria: "${categoria}"`);
    
    // Default modes based on tipo
    const defaultModes = {
        'ALIMENTOS': 'DIARIO',
        'BEBIDAS': 'SEMANAL',
        'INSUMOS': 'MENSUAL'
    ...
    return defaultModes[tipo] || 'SEMANAL'; // Default to SEMANAL if tipo not found
}

// Update the updateModeAndDate function to use the cache
function updateModeAndDate() {
    const tipo = document.getElementById('tipoInventario').value;
    ...
    // Get mode directly from cache - no API call needed
    const modoInventario = getModoFromCache(tipo, categoria);
    console.log(`Found modo: "${modoInventario}"`);
    
    // Set the mode value immediately
